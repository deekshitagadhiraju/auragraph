        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 0, borderRadius: 10, overflow: 'hidden', border: '1.5px solid var(--border)', background: 'var(--bg)', transition: 'border-color 0.15s' }}
                ...
                <button onClick={addUrl} disabled={!url.trim() || status === 'loading'}
                    style={{ padding: '0 14px', background: 'linear-gradient(135deg,#2563EB,#1D4ED8)', color: '#fff', border: 'none', cursor: url.trim() && status !== 'loading' ? 'pointer' : 'not-allowed', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, opacity: !url.trim() ? 0.5 : 1, transition: 'opacity 0.15s' }}>
                    <Plus size={13} /> Add
                </button>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {files.length > 0 && status !== 'ok' && (
                <button onClick={handleTranscribe} disabled={status === 'loading'}
                    style={{ padding: '10px 0', borderRadius: 8, background: 'linear-gradient(135deg,#0F766E,#0D9488)', color: '#fff', border: 'none', cursor: status === 'loading' ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            <div style={{ position: 'relative' }}
                ...
                {!text && (
                    <button onClick={() => ref.current?.click()}
                        style={{ position: 'absolute', bottom: 12, right: 12, padding: '5px 10px', borderRadius: 6, background: '#FFFBEB', border: '1px solid #FCD34D', cursor: 'pointer', fontSize: 11, color: '#B45309', display: 'flex', alignItems: 'center', gap: 5, fontWeight: 700 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button onClick={handleIngest} disabled={loading}
                            style={{ padding: '6px 14px', borderRadius: 7, background: 'linear-gradient(135deg,#D97706,#B45309)', color: '#fff', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div>
            ...
            {hasAnyInput && (
                <div style={{ marginTop: 16, padding: '10px 12px', borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text3)', textTransform: 'none', letterSpacing: '0.06em', marginBottom: 8 }}>
