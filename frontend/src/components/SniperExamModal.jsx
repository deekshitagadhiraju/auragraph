...
export default function SniperExamModal({ nodes, notebookId, onClose, onQuizCompleted }) {
    ...
    const scoreSaved = React.useRef(false);
    ...
    const handleSelect = (opt) => {
        if (revealed) return;
        setSelected(opt);
        setUserResponses(prev => ({ ...prev, [qIdx]: opt }));
        setRevealed(true);
    ...
    const persistFinalScore = React.useCallback((finalScore, finalTotal, responsesPayload) => {
        if (scoreSaved.current || finalTotal <= 0 || !notebookId) return;
        ...
        fetch(`${API}/api/notebooks/${notebookId}/quizzes/complete`, {
            ...
            body: JSON.stringify({
                test_type: 'sniper',
                questions,
                concepts_tested: redNodes.map(n => ({ label: n.label, status: 'struggling' })),
    ...
    const next = () => {
        if (qIdx + 1 >= total) {
            ...
            const responsesPayload = questions.map((q, idx) => ({
                question_index: idx,
                selected_option: (userResponses[idx] || '').toUpperCase(),
                correct_option: (q?.correct || '').toUpperCase(),
                is_correct: ((userResponses[idx] || '').toUpperCase() === (q?.correct || '').toUpperCase()),
            }));
            setScore(finalScore);
            persistFinalScore(finalScore, total, responsesPayload);
            setDone(true);
            return;
        }
        setQIdx(i => i + 1);
