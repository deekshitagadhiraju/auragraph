        ...
        <div style={{ background: 'var(--bg)', borderRadius: 12, border: '1px solid var(--border)', boxShadow: 'var(--shadow-md)', margin: '0 10px 10px', overflow: 'hidden' }}>
            <div style={{ padding: '11px 14px 9px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', background: isDark ? 'var(--surface)' : 'var(--ag-purple-bg)' }}>
                <div>
                    ...
                    <div style={{ fontSize: 10, color: statusColors[node.status] || '#9CA3AF', fontWeight: 600, marginTop: 2, textTransform: 'none' }}>&#9679; {node.status ? node.status.charAt(0).toUpperCase() + node.status.slice(1) : ''} — Tap A Difficulty To Practice</div>
                </div>
            ...
            <div style={{ padding: '10px 14px', overflowY: 'auto' }}>
                {/* Mastery buttons */}
                <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 5, fontWeight: 600, textTransform: 'none', letterSpacing: 0.5 }}>Set Mastery Level</div>
                ...
                <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 6, fontWeight: 600, textTransform: 'none', letterSpacing: 0.5 }}>🎯 Practice Questions</div>
                ...
                {loadingQ && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 0', fontSize: 12, color: 'var(--text3)' }}>
                        <Loader2 className="spin" size={13} /> Generating {activeLevel ? activeLevel.charAt(0).toUpperCase() + activeLevel.slice(1) : ''} Questions…
                    </div>
                )}
                {questions && !loadingQ && (
                    questions.length === 0
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    {nodes.length > 0 && (
                            <span style={{ fontSize: 9, color: 'var(--text3)', fontWeight: 500 }}>
                                Click A Node To Practice · Drag ↕ To Resize
                        </span>
                    )}
                </div>
                {nodes.length > 0 && (
                    <div style={{ display: 'flex', gap: 5 }}>
                        {[['mastered', 'var(--ag-emerald)', mc], ['partial', 'var(--ag-gold)', pc], ['struggling', 'var(--ag-red)', sc]].map(([k, c, count]) => (
                            <div key={k} style={{ flex: 1, textAlign: 'center', background: c + '15', borderRadius: 5, padding: '3px 4px', border: `1px solid ${c}33` }}>
                                ...
                                <div style={{ fontSize: 8, color: c, textTransform: 'none', fontWeight: 600 }}>{k.charAt(0).toUpperCase() + k.slice(1)}</div>
                            </div>
                        ))}
                    </div>
                )}
                ...
                <div style={{ flex: 1 }}>
                    ...
                    {nodes.length > 0 && (
                        <div style={{ padding: '6px 12px 4px' }}>
                            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text3)', marginBottom: 5, textTransform: 'none', letterSpacing: 0.5 }}>
                                All Concepts
