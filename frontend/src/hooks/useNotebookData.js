...
import { ls_getNotebook, ls_saveNote, ls_addNoteVersion, ls_getNoteVersions, ls_restoreNoteVersion, ls_deleteNoteVersion } from '../localNotebooks';
...
export function useNotebookData(id, deps = {}) {
    ...
    const deleteMutationVersion = useCallback(async (versionId) => {
        if (!versionId) return false;
        try {
            const res = await apiFetch(`${API}/api/notebooks/${id}/versions/${versionId}`, { method: 'DELETE' });
            if (!res.ok) throw new Error('delete version failed');
            return true;
        } catch {
            return ls_deleteNoteVersion(id, versionId);
        }
    }, [id]);
