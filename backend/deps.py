...
async def _llm_mutate(
    ...
    if _is_azure_available():
        for attempt in range(2):
            try:
                ...
                logger.info(
                    "azure_mutate_attempt_%d: mutated_len=%d gap_len=%d answer_len=%d",
                    attempt + 1,
                ...
                logger.warning(
                    "Azure mutation returned empty rewrite (attempt %d/2). note_len=%d doubt_len=%d slide_ctx_len=%d textbook_ctx_len=%d",
                    attempt + 1,
                    len(note_page or ""),
                    len(doubt or ""),
                    len(slide_ctx or ""),
                ...
                logger.warning("Azure mutation failed (attempt %d/2): %s", attempt + 1, e)
    if _is_groq_available():
        try:
            ...
            if not (mutated or "").strip():
                logger.warning(
                    "Groq mutation returned empty rewrite. note_len=%d doubt_len=%d slide_ctx_len=%d textbook_ctx_len=%d",
                    len(note_page or ""),
                    len(doubt or ""),
                    len(slide_ctx or ""),
                    len(textbook_ctx or ""),
