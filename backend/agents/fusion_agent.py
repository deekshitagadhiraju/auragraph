...
class FusionAgent:
    ...
    async def mutate(
        ...
        mutated, gap, answer = self._parse_mutate_response(raw)
        logger.info(
            "azure_mutate_parse: raw_len=%d mutated_len=%d gap_len=%d answer_len=%d has_delimiter=%s",
            len(raw),
