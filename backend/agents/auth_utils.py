...
def _migrate_users_from_json():
    ...
    try:
        ...
        with _conn() as con:
            for u in rows:
                con.execute(
                    "INSERT OR IGNORE INTO users (id, email, password_hash, token, token_issued_at, name) VALUES (?,?,?,?,?,?)",
...
def register_user(email: str, password: str, name: Optional[str] = None) -> Optional[dict]:
    ...
    try:
        with _conn() as con:
            con.execute(
                "INSERT INTO users (id, email, password_hash, token, token_issued_at, name) VALUES (?,?,?,?,?,?)",
...
def _normalise_profile(profile: Optional[dict]) -> dict:
    p = profile or {}
    def _clean(key: str, limit: int = 120) -> str:
        return str(p.get(key, "") or "").strip()[:limit]

    # Keep field names stable for frontend and prompt assembly.
    out = {
        "class_level": _clean("class_level", 80),
        "education_stage": _clean("education_stage", 80),
        "institution": _clean("institution", 120),
        "board_university": _clean("board_university", 120),
        "stream_major": _clean("stream_major", 120),
        "target_exam": _clean("target_exam", 120),
        "learning_goal": _clean("learning_goal", 240),
    }
    return out
...
def get_user_profile(user_id: str) -> dict:
    ...
    try:
        parsed = json.loads(row["profile_json"]) if isinstance(row["profile_json"], str) else {}
    except Exception:
        parsed = {}
...
def update_user_profile(user_id: str, profile: dict) -> dict:
    ...
    with _conn() as con:
        con.execute(
            ...
            """,
            (user_id, json.dumps(merged, ensure_ascii=True), time.time()),
        )
    return merged
