...
def save_images(notebook_id: str, images: list[ExtractedImage],
    ...
    # Windows/OneDrive can transiently lock the directory itself. Avoid deleting
    # the folder and instead clear its contents best-effort.
        ...
        try:
            for name in os.listdir(folder):
                ...
                try:
                    ...
                    logger.warning("slide_images: could not remove locked path %s: %s", child, e)
                except Exception as e:
                    logger.warning("slide_images: failed clearing %s: %s", child, e)
        except Exception as e:
            logger.warning("slide_images: could not enumerate %s for clearing: %s", folder, e)
