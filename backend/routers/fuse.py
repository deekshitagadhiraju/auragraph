...
def _profile_to_student_context(profile: dict) -> str:
    """Compact education context appended to generation prompts."""
    if not profile:
        return ""
    fields = [
        ("Education Stage", profile.get("education_stage")),
        ("Class/Level", profile.get("class_level")),
        ("Institution", profile.get("institution")),
        ("Board/University", profile.get("board_university")),
        ("Stream/Major", profile.get("stream_major")),
        ("Target Exam", profile.get("target_exam")),
        ("Learning Goal", profile.get("learning_goal")),
    ]
    lines = [f"{k}: {str(v).strip()}" for k, v in fields if str(v or "").strip()]
    return "\n".join(lines)


def _merge_proficiency_with_profile(proficiency: str, profile: dict) -> str:
    """Preserve chosen proficiency while nudging the model to the user's class/education level."""
    base = (proficiency or "Practitioner").strip()
    class_level = str((profile or {}).get("class_level") or "").strip()
    education_stage = str((profile or {}).get("education_stage") or "").strip()
    if class_level and education_stage:
        return f"{base} | {education_stage}, {class_level}"
    if class_level:
        return f"{base} | {class_level}"
    if education_stage:
        return f"{base} | {education_stage}"
    return base
...
async def upload_fuse_multi(
    ...
    try:
        ...
        _profile = get_user_profile(user["id"])
    except Exception:
        _profile = {}
    _profile_ctx = _profile_to_student_context(_profile)
    _effective_proficiency = _merge_proficiency_with_profile(proficiency, _profile)
    ...
    if topics:
        try:
            ...
            try:
                ...
                _stud_ctx = ""
            _combined_ctx = ("\n\n".join(x for x in [_stud_ctx, _profile_ctx] if x.strip())).strip()
            fused_note, source = await asyncio.wait_for(
                run_generation_pipeline(topics=topics, topic_contexts=topic_contexts,
                                        proficiency=_effective_proficiency, refine=True,
                                        student_context=_combined_ctx),
    ...
    if not fused_note or len(fused_note.strip()) < 100:
        fused_note = generate_local_note(all_slides_text, all_textbook_text, _effective_proficiency)
        source     = "local"
...
async def upload_fuse_stream(
    ...
    try:
        from agents.auth_utils import get_user_profile
        _stream_profile = get_user_profile(user["id"])
    except Exception:
        _stream_profile = {}
    _stream_profile_ctx = _profile_to_student_context(_stream_profile)
    _stream_effective_proficiency = _merge_proficiency_with_profile(proficiency, _stream_profile)
    ...
    async def event_generator():
        if not topics:
            fallback = generate_local_note(all_slides_text, all_textbook_text, _stream_effective_proficiency)
        ...
        try:
            ...
            pass
        _stream_student_ctx = ("\n\n".join(x for x in [_stream_student_ctx, _stream_profile_ctx] if x.strip())).strip()

        final_note, final_source = "", "local"
        try:
            async for event in run_generation_pipeline_stream(topics, topic_contexts, _stream_effective_proficiency, student_context=_stream_student_ctx):
            ...
            if not final_note:
                final_note = generate_local_note(all_slides_text, all_textbook_text, _stream_effective_proficiency)
                final_source = "local"
...
async def fuse_knowledge(req: FusionRequest, authorization: Optional[str] = Header(None)):
    ...
    try:
        ...
        _profile = {}
    _profile_ctx = _profile_to_student_context(_profile)
    _effective_proficiency = _merge_proficiency_with_profile(req.proficiency, _profile)
    ...
    try:
        ...
        if topics:
            ...
            fused_note, source = await run_generation_pipeline(
                ...
                proficiency=_effective_proficiency, refine=True,
                student_context=_profile_ctx,
            )
    except Exception as exc:
        logger.warning("/api/fuse pipeline failed: %s", exc)

    if not fused_note or len(fused_note.strip()) < 100:
        fused_note = generate_local_note(req.slide_summary, req.textbook_paragraph, _effective_proficiency)
        source     = "local"
