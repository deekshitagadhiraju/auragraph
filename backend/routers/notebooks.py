...
@router.delete("/api/notebooks/{nb_id}/versions/{version_id}")
async def delete_note_version_endpoint(
    nb_id: str,
    version_id: str,
    authorization: Optional[str] = Header(None),
):
    from agents.notebook_store import delete_note_version
    user = get_current_user(authorization)
    _require_notebook_owner(nb_id, user)
    ok = delete_note_version(nb_id, version_id)
    return {"ok": ok}
