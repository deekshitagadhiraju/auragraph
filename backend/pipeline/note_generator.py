...
NOTE_USER_TEMPLATE = """Generate study notes for the following lecture topic.

TOPIC: {topic}

IMPORTANT: If the provided TOPIC heading is incorrect, unclear, or not the best summary for the slide content, replace it with a more accurate and concise heading. Use your own judgment as an expert. The heading you use in your output will be treated as the final, verified heading for this section.
...
async def generate_topic_note(
    ...
    if len(topic.slide_text) <= _SPLIT_THRESHOLD:
        ...
        if result:
            ...
            # --- Extract heading from generated note and update topic if needed ---
            new_heading = _extract_section_heading(result)
            if new_heading and new_heading.strip() and new_heading.strip() != topic.topic.strip():
                topic.topic = new_heading.strip()
    ...
    if total_draft_chars > _MERGE_OUTPUT_CHARS_LIMIT:
        ...
        # --- Extract heading from generated note and update topic if needed ---
        new_heading = _extract_section_heading(combined)
        if new_heading and new_heading.strip() and new_heading.strip() != topic.topic.strip():
            topic.topic = new_heading.strip()
    ...
    if merged and len(merged.strip()) >= max(100, int(total_draft_chars * 0.40)):
        ...
        # --- Extract heading from generated note and update topic if needed ---
        new_heading = _extract_section_heading(merged)
        if new_heading and new_heading.strip() and new_heading.strip() != topic.topic.strip():
            topic.topic = new_heading.strip()
    ...
    if new_heading and new_heading.strip() and new_heading.strip() != topic.topic.strip():
        topic.topic = new_heading.strip()
    return await _ensure_full_coverage(combined, provider, topic, proficiency, api_sem)
# --- Helper: Extract heading from generated note section ---
def _extract_section_heading(note_text: str) -> str | None:
    """
    Extract the main section heading from the generated note (expects '## Heading' at the top).
    Returns the heading text, or None if not found.
    """
    match = re.search(r'^##\s+(.+)', note_text, re.MULTILINE)
    if match:
